<script setup>
  ...
  import {GradeStore} from '@/stores/grade';
  import { onMounted } from 'vue';
  import EditIcon from '@/components/icons/EditIcon.vue';
  import DeleteIcon from '@/components/icons/DeleteIcon.vue';
  import CreateModalComponent from '@/components/InstituteManagement/Grade/CreateModalComponent.vue';
  import EditModalComponent from '@/components/InstituteManagement/Grade/EditModalComponent.vue';
                            ...
                            <div class="border p-4 rounded-lg space-y-3 divide-gray-200">
                                <div class="w-full md:flex justify-between space-y-3 md:space-y-0">
                                    ...
                                    <div class="relative max-w-xs">
                                        ...
                                        <input type="text" name="hs-table-with-pagination-search" v-model="gradeStore.searchInput.search"
                                    ...
                                    <div class="inputDropdown space-x-4">
                                        <button @click="gradeStore.reset()" class="py-2 px-3 rounded-sm bg-[#32a370] hover:bg-[#247752] duration-150 text-white text-[15px]" aria-haspopup="dialog" aria-expanded="false" aria-controls="create-grade" data-hs-overlay="#create-grade">Create New</button>
                                    </div>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="w-full divide-y divide-gray-400 border border-gray-400">
                                        ...
                                        <tbody class="divide-y divide-gray-400 dark:divide-gray-700">
                                            <tr v-for="(grade, index) in gradeStore.grades" :key="index">

                                                <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-400">{{ index+1 }}</td>
                                                <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200 capitalize">{{ grade.name }}</td>

                                                <td class="px-6 py-2 flex gap-4">
                                                    <button @click ="gradeStore.deletegrade(grade.id)" type="button" class="py-1.5 px-1.5 bg-red-500 hover:bg-red-600 rounded-md">
                                                        <DeleteIcon class="text-white text-[18px]"/>
                                                    </button>
                                                    <button @click="gradeStore.GradeEdit(grade.id)" type="button" class="py-1.5 px-1.5 bg-blue-600 hover:bg-[#2b4988] rounded-md">
                                                        <EditIcon class="text-white text-[18px]"/>
                                                    </button>
                                                </td>
                                            </tr>

                                            <tr v-if="!gradeStore.grades?.length > 0 && !gradeStore.isLoading">
                                                <td class="text-gray-600 dark:text-gray-100 text-center text-sm py-3" colspan="8">No data found</td>
                                            </tr>

                                            <tr v-if="gradeStore.isLoading">
